...
Embodied diagnostic systems extend governance beyond the output of an isolated model: they observe people longitudinally, intervene in physical or clinical workflows and depend on connected infrastructures. Governance should therefore scale with the persistence of sensing, the authority delegated to the system, the reversibility of its actions and the consequences of system-level failure.


\subsection{Privacy and autonomy in continuous detection}
\label{subsec:privacy_autonomy}
Continuous sensing turns privacy from an episodic disclosure into a condition of inhabiting a monitored space. Ambient, wearable and implanted systems may infer routines, relationships or sensitive health states beyond the intended biomarker, while also capturing family members, visitors or staff. Because unobtrusive sensing can make observation difficult to perceive or avoid, improved convenience does not by itself justify expanded collection. Consent should instead be treated as an ongoing and revisable process: people should understand which raw signals and derived inferences are generated, how they will be used and shared, and how monitoring can be paused or withdrawn without inappropriate loss of care. Remote monitoring of implantable devices further shows that access to device-derived data and responsibility for its stewardship may be divided among manufacturers, healthcare institutions and patients. Data minimization, local processing where feasible and protected access can reduce exposure, but autonomy also requires visible monitoring states, private periods, alternatives for those who decline, and safeguards against coercive use. Continuous risk estimates should support rather than displace the patient's own account and preferences, consistent with WHO's emphasis on autonomy and accountability across public and private actors.


\subsection{Safety and accountability in diagnostic action}
\label{subsec:safety_accountability}
The ethical stakes increase when detection selects a test, guides physical contact, acquires a specimen or initiates escalation. A mistaken inference can then propagate into delayed diagnosis, inappropriate care or direct physical harm. Safety must therefore be assessed across the complete sensing--inference--action loop, including foreseeable interactions among measurement error, software, hardware, users and environment. ISO 14971 provides a lifecycle framework for identifying hazards, controlling risks and evaluating residual risk, but the required evidence and safeguards should become more stringent as autonomy and the severity or irreversibility of possible harm increase. Keeping a clinician nominally ``in the loop'' is insufficient. Oversight is protective only when the supervisor has the knowledge, cognitive space, decision authority and effective means to intervene before harm occurs. These conditions imply a bounded operating domain, calibrated abstention and confirmation rules, enforceable physical and software limits, safe interruption and traceable logs. Accountability should be distributed but explicit: developers and manufacturers remain responsible for design and update controls; institutions for local validation, staffing and workflow integration; and operators for decisions within their actual competence and authority. Auditable decision rights and feedback pathways are needed to prevent residual responsibility from being shifted to the final clinician.


\subsection{Equity and resilience of connected systems}
\label{subsec:equity_resilience}
Inequity can enter before model training and persist after deployment. Pulse oximetry illustrates bias at the physical measurement interface, whereas the use of healthcare expenditure as a proxy for health need shows how an apparently accurate objective can reproduce structural inequality. Evaluation should therefore follow the entire pathway from sensor fit and signal quality to labels, model outputs, clinician response, access and realized benefit, with performance examined across relevant and intersecting population and site groups. Health-data poverty, inaccessible interfaces, connectivity costs and unequal capacity for maintenance can exclude populations or make an intervention preferentially benefit already advantaged users. Resilience is consequently an equity issue as well as an engineering property. WannaCry disrupted appointments, admissions and emergency activity in affected hospitals, demonstrating that failure of connected infrastructure can interrupt care even when the diagnostic model itself remains unchanged. Secure update and recovery mechanisms, offline-safe operation and workable manual fallbacks should therefore be treated as patient-safety requirements throughout the device lifecycle. Subgroup monitoring, incident learning and contingency planning should explicitly account for poorly resourced sites and users with the least redundancy, who are often most exposed to both performance gaps and service interruption.


\section{Outlook: Three Grand Challenges}
\label{sec:future_outlook}
The next phase of embodied diagnostics will depend less on isolated gains in sensing or prediction than on closing feedback loops at three scales: the diagnostic workflow, the individual patient and the wider care system.


\subsection{Self-optimizing diagnostic workflows}
\label{subsec:self_optimizing_diagnostic_workflows}
Self-driving laboratories provide an architectural precedent in which an objective is translated into experiments, measurements update a model and the model selects the next experiment. Chemistry and materials platforms have closed this loop through mobile robotics, Bayesian optimization and autonomous adjustment of experimental conditions. The diagnostic counterpart would actively select the next question, imaging view, laboratory test or sampling action according to its expected clinical value, while accounting for uncertainty, delay, cost, invasiveness and acquisition failure. Conversational and EHR-based agents already demonstrate components of sequential information gathering and workflow execution. However, these studies have predominantly used simulated consultations or sandboxed clinical environments; they do not establish reliable physical acquisition or autonomous clinical practice. The central challenge is therefore to couple value-of-information reasoning to robust sensing and manipulation under explicit human authority. Success should be judged not only by diagnostic accuracy, but also by time to an adequate decision, unnecessary tests, failed acquisitions, patient burden, clinician interventions and harm.


\subsection{Patient-specific diagnostic models}
\label{subsec:patient_specific_embodied_models}
Population models interpret patients relative to cohort averages, whereas longitudinal studies show that clinically meaningful changes may first appear as deviations from an individual's own baseline. Personal omics, dense phenotyping and host--microbiome profiling have revealed substantial within-person dynamics during infection, metabolic change and disease evolution. Wearable data can similarly predict laboratory measurements or detect presymptomatic deviations from personalized physiological baselines. A future patient-specific model should integrate clinical records, imaging, molecular measurements and continuous sensors to estimate a changing latent health state and determine which observation would most reduce consequential uncertainty. Such a model becomes an embodied diagnostic component only when its predictions guide new physical measurements or actions and are updated by their consequences. The major challenge is to distinguish disease-related change from biological variability, behavioural context, sensor drift and treatment effects while learning from sparse, irregular and missing observations. Digital-twin terminology should therefore be reserved for dynamically updated, patient-linked models with demonstrated predictive or decision value, rather than static dashboards or retrospective simulations. Validation must ultimately show prospective within-patient calibration and improved decisions, not merely reconstruction of historical trajectories.


\subsection{Detection--intervention ecosystems}
\label{subsec:closed_loop_detection_intervention}
Existing systems illustrate markedly different levels of maturity. Automated insulin delivery has multicentre randomized evidence, whereas biomarker-triggered neuromodulation for depression remains an important single-patient demonstration. Integrated sensing--delivery materials and wearable bidirectional neural interfaces provide complementary device-level capabilities, but not yet general clinical ecosystems. The grand challenge is to connect detection, intervention and reassessment across devices, people and care settings without allowing an error in one component to propagate through the loop. This requires models that estimate intervention effects, optimize when and how strongly to act, distinguish treatment response from natural fluctuation and detect mismatch between the patient, sensor and controller. Components must remain independently testable, with bounded actions, safe degraded states, human override and recovery from delayed, missing or contradictory feedback. FDA guidance accordingly treats sensor properties, control algorithms, delivery mechanisms, automation level, users and environment as interdependent design variables. At health-system scale, device feedback must connect to learning cycles that convert data into knowledge, practice improvement and new data while preserving clinical oversight. Across all three challenges, the decisive criterion is not whether a loop operates autonomously, but whether feedback produces safer and more informative patient-relevant decisions.
